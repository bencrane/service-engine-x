...
- **`bookingFields`** (`object | object | object[]`): Custom fields that can be added to the booking form when the event is booked by someone. By default booking form has name and email field.
...
- **`calVideoSettings`** (`object`): Cal video settings for the event type. Platform customers can't manage this property because currently we have no way of determining if managed user is a host or an attendee.
- **`hidden`** (`boolean`)
...
- **`showOptimizedSlots`** (`boolean`): Arrange time slots to optimize availability.
  - Default: `False`
- **`schedulingType`** **required** (`enum`): The scheduling type for the team event - collective, roundRobin or managed.
  - Enum values: `collective`, `roundRobin`, `managed`
  - Example: `collective`
