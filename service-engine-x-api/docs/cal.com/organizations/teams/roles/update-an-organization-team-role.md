...
- **`permissions`** (string (enum)[]) *optional*
...
```bash
curl -X PATCH "https://api.cal.com/v2/organizations/{orgId}/teams/{teamId}/roles/{roleId}" \
  -H "Authorization: Bearer <token>" \
  -H "cal-api-version: 2024-08-13" \
  -H "Content-Type: application/json" \
  -d '{"color": "string", "description": "string", "permissions": ["eventType.read", "eventType.create", "booking.read"], "name": "string"}'
```
