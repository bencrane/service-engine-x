...
import { callSerxApi } from "./http-client.js";
...
export function annotationFromKind(kind) {
    if (kind === "RO") {
        return {
            readOnlyHint: true,
            idempotentHint: true,
...
function applyPathParams(path, input) {
    let nextPath = path;
...
function splitPayload(def, input) {
    ...
    if (def.allowBody && body === undefined) {
        ...
        for (const [key, value] of Object.entries(input)) {
            if (key === "query" || key === "body" || tokens.has(key))
                continue;
...
export function isDestructiveBlocked(def) {
    return def.kind === "DEST" && !env.allowDestructiveTools;
}
export async function executeCoreTool(def, input) {
    if (isDestructiveBlocked(def)) {
        const destructiveBlockedError = {
            category: "validation_error",
            message: "Destructive tools are disabled. Set SERX_ALLOW_DESTRUCTIVE_TOOLS=true for an explicit controlled run.",
            retryable: false,
            status: 403,
        };
        return {
...
export function getToolInputSchema(def) {
