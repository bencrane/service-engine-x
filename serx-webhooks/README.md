...
`PORT` is set by Railway at the platform level — not Doppler. The Dockerfile CMD is `sh -c "doppler run -- uvicorn app.main:app --host 0.0.0.0 --port ${PORT}"`; `${PORT}` is shell-expanded before `doppler run`, so uvicorn binds to Railway's value. `PORT` is intentionally **not** in `app/config.py`, so a missing `PORT` in Doppler will not fail boot.
